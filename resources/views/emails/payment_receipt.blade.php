...
<head>
    ...
    <style>
        ...
        /* Card */
        .card { background-color: #f3f4f6; border-radius: 12px; padding: 24px; margin-bottom: 24px; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <!-- Using Snapay logo but styled like OUK header -->
            <img src="{{ $message->embed(public_path('Snapay.png')) }}" alt="Snapay">
        </div>

        <div class="content">
            <div class="greeting">Your Receipt</div>
            
            <div class="date-info">
                {{ optional($receipt->issued_at)->format('F j, Y') }}<br>
                Order ID: {{ $receipt->receipt_number }}
            </div>
