    ...
    <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        @auth
            @if (Auth::user()->is_admin)
                ...
                <a href="{{ route('admin.smtp.index') }}" class="flex items-center px-3 py-2 rounded-md hover:bg-gray-100 {{ request()->routeIs('admin.smtp.*') ? 'text-secondary bg-gray-50' : 'text-primary hover:text-secondary' }}">SMTP</a>
