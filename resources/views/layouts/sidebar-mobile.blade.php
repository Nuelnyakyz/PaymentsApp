    ...
    <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        @auth
            @if (Auth::user()->is_admin)
                ...
                <a href="{{ route('admin.services.index') }}" @click="$store.ui.mobileSidebar = false" class="flex items-center px-3 py-2 rounded-md hover:bg-gray-100 {{ request()->routeIs('admin.services.*') ? 'text-secondary bg-gray-50' : 'text-primary hover:text-secondary' }}">Services</a>
                <a href="{{ route('admin.users.index') }}" @click="$store.ui.mobileSidebar = false" class="flex items-center px-3 py-2 rounded-md hover:bg-gray-100 {{ request()->routeIs('admin.users.*') ? 'text-secondary bg-gray-50' : 'text-primary hover:text-secondary' }}">{{ __('nav.users') }}</a>
            @endif
        @endauth
    </nav>
</aside>
